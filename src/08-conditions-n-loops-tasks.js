...
function doRectanglesOverlap(r1, r2) {
  const isCoordsOverlaps = (a1, a2, b1, b2) => (a1 < b1 && b1 < a2) || (a1 < b2 && b2 < a2)
    || (b1 < a1 && a1 < b2) || (b1 < a2 && a2 < b2);
  return isCoordsOverlaps(r1.left, r1.left + r1.width, r2.left, r2.left + r2.width)
    && isCoordsOverlaps(r1.top, r1.top + r1.height, r2.top, r2.top + r2.height);
}
...
function findFirstSingleChar(str) {
  try {
    return str.split``.filter((ch, _, arr) => arr.filter((el) => el === ch).length === 1)[0];
  } catch (e) {
    return null;
  }
}
...
function getCommonDirectoryPath(pathes) {
  const pths = pathes.map((path) => path.split``);
  let result = '';

  pths[0].some((ch, i) => {
    if (pths.every((el, j, arr) => arr[j][i] === ch)) {
      result += ch;
      return false;
    }
...
function evaluateTicTacToePosition(position) {
  ...
  const mat2str = (arr) => arr.map((el) => el.join``).join` `;
  const m = JSON.parse(JSON.stringify(position).replace(/null/g, '" "'));
  const testStr = [
    mat2str(m),
    mat2str(transpose(m)),
    `${m[0][0]}${m[1][1]}${m[2][2]}`,
    `${m[0][2]}${m[1][1]}${m[2][0]}`].join` `;

  if (/XXX/.test(testStr)) return 'X';
  if (/000/.test(testStr)) return '0';
  return undefined;
}
